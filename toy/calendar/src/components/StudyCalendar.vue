          ...
          <tr v-for="(date, idx) in dates" :key="idx" class="day-container">
            <td
              ...
              :class="[
                ...
                `orange-${convertRunningTimeToColorLevel(
                  convertMsToHour(studyTimeCalendar[day].runningTime)
                )}`,
              ]"
            ...
            >
              ...
              <div class="tooltip-box">
                ...
                <div class="day-container" :key="selectedDate">
                  <div
                    v-for="(hour, index) in selectedDateStudyHourList"
                    :key="'detail' + index"
                    class="day-detail-box"
                    :class="{ 'has-study-hour': hour }"
                  >
                    {{ index }}
                  </div>
                </div>
...
export default {
  ...
  watch: {
    month: {
      ...
      handler(value) {
        ...
        this.studyTimeCalendar = this.convertStudyTimeMonthToCalendar(
          this.getStudyTimeMonth(this.year, value)
        );
        console.log(
          '%cStudyCalendar.vue line:104 this.studyTimeCalendar',
          'color: #007acc;',
          this.studyTimeCalendar
        );
      },
    },
  },
  ...
  methods: {
    ...
    getStudyTimeMonth(year, month) {
      ...
      console.log(
        '%cStudyCalendar.vue line:296 year+',
        '+month',
        'color: #007acc;',
        year + ',' + month
      );
